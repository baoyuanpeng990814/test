    ...
    <div class="body_no_top ofs">
      <v-alert color="#F0F1F6" minWidth="100%">
        <div class="i-row i-align-center i-between">
          剩余时间: {{ remnantTime | transformTime }}
          <div class="i-row">
...
export default {
  ...
  methods: {
    ...
    submitAnswer() {
      if (this.answerMore) {
        this.answer = this.answerMore.toString()
      }
      this.$store
        .dispatch('saveAnswer', {
    ...
    previousQuestion() {
      ...
      if (this.answerMore) {
        this.answerMore = this.saveAnswerMore
      }
      this.submitAnswer()
    ...
    nextQuestion() {
      ...
      // 记录上次多选选项
      if (this.answerMore) {
        this.saveAnswerMore = this.answerMore
        this.answerMore = []
      }
      this.submitAnswer()
