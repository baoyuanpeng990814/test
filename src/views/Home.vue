  ...
  <div class="home_page">
    ...
    <v-bottom-navigation v-if="show" color="#19B6CA" grow value="index">
      <v-btn to="MenuIndex" value="1" min-width="60px" @click="set(1)">
        ...
        <v-img
          src="../images/icon/icon_home.png"
          max-height="1.25rem"
          max-width="1.25rem"
          contain
          :class="index == 1 ? 'change1' : 'change2'"
        ></v-img>
      </v-btn>
      ...
      <v-btn to="MyCourses" value="2" min-width="60px" @click="set(2)">
        ...
        <v-img
          src="../images/icon/icon_course2.png"
          max-height="1.25rem"
          max-width="1.25rem"
          contain
          :class="index == 2 ? 'change1' : 'change2'"
        ></v-img>
      </v-btn>
      ...
      <v-btn to="MyExams" value="3" min-width="60px" @click="set(3)">
        ...
        <v-img
          src="../images/icon/icon_exam2.png"
          max-height="1.25rem"
          max-width="1.25rem"
          contain
          :class="index == 3 ? 'change1' : 'change2'"
        ></v-img>
      </v-btn>
...
export default {
  ...
  created() {
    if (document.cookie) {
      this.getCookie()
      // this.$store.commit("", JSON.parse(document.cookie))
      this.index = this.$store.state.index
      if(this.$router.currentRoute.path=='/coursedetails') {
        this.show=false
      }
      console.log(this.$router.currentRoute.path, 'this')
    }
  },
  mounted() {
