<template>
	<v-main>
		...
		<v-toolbar color="white" >
			...
			<div class="tool_title_center">竞赛答题</div>
			<v-spacer></v-spacer>
	...
	export default {
		...
		methods: {
			submitAnswer() {
				...
				this.$store.dispatch("competitionSaveAnswer", {
			...
			getExam() {
				// 获取考试详情信息
				this.$store.dispatch("competitionDetailInfo", this.id).then((res) => {
					if (res.data.state == 200) {
			...
			submit() {
				...
				this.$store.dispatch("competitionSave",{
