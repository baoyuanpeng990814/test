<template>
  <div class="exam_panel">
    <div class="exam_head">
      ...
      <div>剩余时间: {{ remnantTime | transfermTime }}</div>
    </div>
        ...
        <div>
          <el-card class="box-card question_torso">
            ...
            <div>
              ...
              <div>
                <span v-if="questions[questionIndex].questionsType == 2">
                  <el-checkbox-group v-model="answer">
                    ...
                    >
                      <div>
                        <el-checkbox :label="l.optionNum" :key="l.optionNum">{{
                          l.optionNum
                        }}</el-checkbox>
                      </div>
                      <div>
                ...
                <span v-else>
                  ...
                  >
                    <div>
                      <el-radio v-model="answer" :label="l.optionNum">{{
                        l.optionNum
                      }}</el-radio>
                    </div>
                    <div>
              ...
              <div>
                <el-button type="primary" @click="previousQuestion"
                  >上一题</el-button
                >
...
export default {
  ...
  beforeDestroy() {
    ...
    if (!this.timeout) {
      if (!this.once) {
        this.$http.get(
          '/learn/myexam/save?examId=' +
            this.datadetail.exam.examId +
            '&examUserId=' +
            this.datadetail.examUserId +
            '&examTime=' +
            this.remnantTime
        )
      }
  ...
  methods: {
    ...
    async submitAnswer() {
      ...
      const { data: res } = await this.$http.post('/learn/myexam/saveAnswer', {
    ...
    async getExam() {
      const { data: res } = await this.$http.get(
        '/learn/myexam/examInfo?id=' + this.enrollId
      )
      if (res.state !== 200) {
        ...
        this.remnantTime = this.translateTime(
          parseInt(this.datadetail.exam.timingStartTime)
        )
        this.questions = []
        ...
        if (this.datadetail.examQuestions.check != null) {
          // 分割出多选题
          for (let i = 0; i < this.datadetail.examQuestions.check.length; i++) {
        ...
        if (this.datadetail.examQuestions.judge != null) {
          // 分割出判断题
          for (let i = 0; i < this.datadetail.examQuestions.judge.length; i++) {
