          ...
          <div>
            <el-card>
              ...
              <div class="selection_page">
                <el-row>
                  <el-col :span="6" v-for="c in currents" :key="c.id">
                    <!-- {{s.courseName}}-->
                    ...
                    <div class="item_div_left">
                      ...
                      <div class="item_info">
                        ...
                        {{ c.createTime | truncateTime }}---{{
                          c.failureTime | truncateTime
                        }}
                      </div>
                    </div>
                  </el-col>
  ...
  methods: {
    async goExam() {
      if (this.currentdata.countType >= this.currentdata.examSubmit) {
        ...
        const { data: res } = await this.$http.post('/learn/exam/enrollExam', {
    ...
    async getExamTop() {
      const { data: res } = await this.$http.post('/learn/exam/rankingExam')
      if (res.state !== 200) {
        //return this.$message.error('数据获取失败！')
      } else {
        this.tops = res.data
    ...
    async getCurrentExam() {
      const { data: res } = await this.$http.post('/learn/exam/progressExam')
      if (res.state !== 200) {
        //return this.$message.error('数据获取失败！')
      } else {
    ...
    async getComingExam() {
      const { data: res } = await this.$http.post('/learn/exam/aboutExam')
      if (res.state !== 200) {
        //return this.$message.error('数据获取失败！')
      } else {
