          ...
          <el-container class="acquisition">
            <el-main>
              ...
              <div class="video_info">
                ...
                <div>
                  <div class="space"></div>
                  <input
                    @keyup.enter="takeNote"
                    class="note"
                    prefix-icon="el-icon-edit-outline"
                    v-model="note"
                    placeholder="记笔记,请输入想要记录的内容.按Enter保存笔记"
                  />
                  <div class="space"></div>
                </div>
              </div>
            ...
            <el-aside width="300px">
              <div class="item_n_note">
                ...
                <span v-else>
                  <ul class="note_list">
                    <li v-for="n in notes.length" :key="n.id">
                      <div class="time_reminder">
                        <i class="el-icon-info"></i>
                        <span
                          ><i class="el-icon-caret-left"></i
                          >{{ notes[n - 1].studyNotesTime }}</span
                        >
                        <span>
                          <i class="el-icon-edit" @click="editNote(n - 1)"></i>
                          <i
                            class="el-icon-delete"
                            @click="deleteNote(notes[n - 1])"
                          ></i>
                        </span>
                      </div>
                      <span v-if="!noteEdit[n - 1]">{{
                        notes[n - 1].studyNotes
                      }}</span>
                      <span v-if="noteEdit[n - 1]">
                        <el-input v-model="notes[n - 1].studyNotes"></el-input>
                        <div class="space"></div>
                        <el-button
                          @click="confirmEditNote(notes[n - 1])"
                          type="primary"
                          size="mini"
                          >修改</el-button
                        >
                        <el-button
                          @click="cancelEditNote(n - 1)"
                          type="danger"
                          size="mini"
                          >取消</el-button
                        >
          ...
          <div class="total_rate">
            ...
            <div>
              ...
              <el-radio v-model="filterRate" :label="3"
                >差评({{ comments_bed.length }})</el-radio
              >
...
export default {
  ...
  methods: {
      ...
      if (res.state !== 200) {
        ...
        for (var i = 0; i < this.comments.length; i++) {
          ...
          if (this.comments[i].evaluateType === '1')
            ...
            this.comments_bed.push(this.comments[i])
    ...
    async confirmEditNote(n) {
      // 提交编辑笔记
      const { data: res } = await this.$http.post('/learn/snotes/edit', {
    ...
    async deleteNote(n) {
      // 删除笔记
      const { data: res } = await this.$http.post('/learn/snotes/del', {
        studyNotesId: n.studyNotesId
      })
      if (res.state !== 200) {
        // return this.$message.error('数据获取失败！')
    ...
    async setNote() {
      // 添加笔记
      const { data: res } = await this.$http.post('/learn/snotes/add', {
        chapterId: this.currentChapter.chapterId,
        courseId: this.id,
    ...
    async getCourse() {
      const { data: res } = await this.$http.get(
        '/learn/myLearn/courseInfo?id=' + this.id
      )
      if (res.state !== 200) {
        // return this.$message.error('数据获取失败！')
      } else {
      ...
      if (res.state !== 200) {
        ...
        if (this.currentChapter.lLearn == null) {
          ...
          document.getElementById(
            'video'
          ).currentTime = this.currentChapter.lLearn.learnTime

          var t = this
          var xt = setInterval(function() {
            var duration =
              document.getElementById('video').duration.toFixed(0) - 1
            // console.log(duration)
            if (duration != 0 && !isNaN(duration)) {
              t.currentChapter.done =
                document.getElementById('video').currentTime.toFixed(0) >=
                duration
              t.$forceUpdate()
              clearInterval(xt)
            }
  ...
  created() {
    ...
    si = setInterval(function() {
      t.currenttime =
        document.getElementById('video').currentTime.toFixed(0) - 1
