<template>
  <div class="selection_confirm">
    <div class="sc_head">
      ...
      <div class="showing">
        <div>
          <el-image :src="paperdetail.courseCover" style="height: 285px; width: 100%;"></el-image>
        </div>
        <div>
          ...
          <div class="p_button" v-else>
            ...
            <el-button @click="enterCourse" class="mt15" type="warning">进入课程</el-button>
          </div>
        ...
        <el-container>
          <el-main>
            <el-tabs type="border-card">
              <el-tab-pane label="课程列表">
                <ul class="chapter_list">
                  <li v-for="c in chapters">
                    <i class="el-icon-video-play"></i>&nbsp;&nbsp;{{c.chapterName}}
                  </li>
                </ul>
              </el-tab-pane>
              <el-tab-pane label="学员评论">
                ...
                <div class="total_rate">
                  ...
                  <div>
                    ...
                    <el-radio v-model="filterRate" :label="3">差评({{comments_bad.length}})</el-radio>
              ...
              <el-tab-pane label="课程简介">{{paperdetail.description}}</el-tab-pane>
            </el-tabs>
          </el-main>
  ...
  export default {
    ...
    methods: {
      ...
      async submitEnrollment() {
        const {
          data: res
        } = await this.$http.post("/learn/course/enroll", {
          courseId: this.id,
      ...
      async getComment() { //获取评论
        ...
        if (res.state !== 200) {
          ...
          for (var i = 0; i < this.comments.length; i++) {  //把评论拆分为，好评，中评，差评，我的评论
            ...
            else if (this.comments[i].evaluateType == "3") this.comments_bad.push(this.comments[i])
      ...
      async getPaperDetail() { //获取课程信息
        const {
          data: res
        } = await this.$http.get("/learn/course/courseInfo?id=" + this.id)
        if (res.state !== 200) {
          //return this.$message.error('数据获取失败！')
        } else {
      ...
      async getChapters() { //获取课程章节
        const {
          data: res
        } = await this.$http.post("/learn/course/courseLChapter", {
          courseId: this.paperdetail.id
        })
        if (res.state !== 200) {
          //return this.$message.error('数据获取失败！')
    ...
    watch: {
      filterRate() {
        ...
        else if (this.filterRate == 3) this.current_comments = this.comments_bad
