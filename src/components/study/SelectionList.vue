            ...
            <div class="text item">
              <el-collapse>
                <el-collapse-item :title="currentTheme.themeName" name="1">
                  <el-button
                    @click="changeParentTheme(t.themeId)"
                    v-for="t in themes"
                    plain
                    :key="t.id"
                    >{{ t.themeName }}</el-button
                  >
                </el-collapse-item>
              </el-collapse>
              ...
              <div class="selection_page mt20">
                <el-button
                  @click="changeParentTheme(currentTheme.themeId)"
                  plain
                  >全部</el-button
                >
                <el-button
                  @click="changeParentTheme(t.themeId)"
                  class="t.themeId == id ? 'unchose_btn' :  'chose_btn'"
                  v-for="t in currentTheme.chilLTheme"
                  plain
                  :key="t.id"
                >
                  {{ t.themeName }}
              ...
              <div class="selection_page">
                <el-row>
                  ...
                  <el-col :span="6" v-for="s in sections" :key="s.id">
                    <!-- {{s.courseName}}-->
...
export default {
  ...
  created() {
    ...
    var _this = this
    setTimeout(function() {
      if (cid != null) _this.changeParentTheme(cid)
    }, 50)
  }
}
</script>
