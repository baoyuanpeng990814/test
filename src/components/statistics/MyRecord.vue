  ...
  <div>
    <el-tabs type="border-card">
      ...
      <el-tab-pane label="考试记录">
        <el-table height=" calc(100vh - 150px) " ref="multipleTable" :data="tableData" tooltip-effect="dark"
          style="width: 100%">
          <el-table-column prop="examName" label="考试名称" width="120">
          </el-table-column>
          <el-table-column prop="examTime" label="报考时间" width="170">
            <template slot-scope="scope">{{ scope.row.examTime |transfermDate }}</template>
          </el-table-column>
        ...
        <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :page-sizes="[1, 2, 5, 8]"
          :page-size="query.rows" layout="total, prev, pager, next, jumper" :total="total">
        </el-pagination>
      </el-tab-pane>
...
<script>
  export default {
    ...
    methods: {
      ...
      async getExamReport() { //统计考试分数
        const {
          data: res
        } = await this.$http.post("/learn/myexam/myExamlist", this.query)
        if (res.state !== 200) {
