    ...
    <el-container>
      <el-aside width="200px" class="question_list">
        <div v-if="datadetail.lExamineQuestions != null">
          <ul>
            <li v-for="q in questions.length" :key="q.id" @click="checkThis(questions[q - 1])" :class="{ done: questions[q - 1].done }">
              {{ q }}
            </li>
          </ul>
          <div class="cb"></div>
        ...
        <div>
          <el-card class="box-card question_torso">
            ...
            <div v-if="questions.length > 0">
              ...
              <div>
                <span>
                  <li :key="l.id" v-for="l in questions[questionIndex].examineQuestionOption">
                    <div>
  ...
  methods: {
    async submit() {
      ...
      for (var i = 0; i < this.questions.length; i++) {
        answers +=
          this.questions[i].examineQuestionId +
          ':' +
    ...
    async getExam() {
      ...
      const { data: res } = await this.$http.get(
        '/learn/examins/examineInfo?examineId=' + this.id
      )
      if (res.state !== 200) {
        ...
        for (var i = 0; i < this.datadetail.lExamineQuestions.length; i++) {
          ...
          var temp = {
            ...
            examineQuestionId: this.datadetail.lExamineQuestions[i]
              .examineQuestionId,
