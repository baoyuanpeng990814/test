        ...
        <el-main>
          <div>
                  ...
                  <el-col :span="6" v-for="c in currents" :key="c.id">
                    ...
                    <div class="item_div_left">
                      ...
                      <div class="item_info">
                        ...
                        {{ c.startTime | truncateTime }}---{{
                          c.endTime | truncateTime
                        }}
                      </div>
                    </div>
                  </el-col>
            ...
            <el-card>
              ...
              <div class="selection_page">
                <el-row>
                  <el-col :span="6" v-for="c in coming" :key="c.id">
                    <!-- {{s.courseName}}-->
                    <div class="item_div_left">
                      <el-image :src="c.examineCover"></el-image>
                    </div>
...
export default {
  ...
  methods: {
    ...
    async getCurrentExam() {
      const { data: res } = await this.$http.post('/learn/examins/list')
      if (res.state !== 200) {
        // return this.$message.error('数据获取失败！')
      } else {
    ...
    async getComingExam() {
      const { data: res } = await this.$http.post('/learn/examins/aboutExamine')
      if (res.state !== 200) {
        // return this.$message.error('数据获取失败！')
      } else {
