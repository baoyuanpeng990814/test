<template>
	<div class="i-column">
		<div class="swiper-component" @touchstart="onTouchStart" @touchend="onTouchEnd" :style="{height:height}">
			<ul :style="[ulStyle, swiperStyle]" style="display: flex; flex-direction: row;">
				<li v-for="(item, ind) in list" :key="ind" :style="{width: itemWidth + 'px' } "
					:class="[index==ind?'active':'default']" @click="handleClick(item)" style="list-style: none;">					
					<img :src="item"  style="object-fit: cover"/>
					<!-- <v-img :src="item" :style="{height:height}"></v-img> -->
				</li>
			</ul>
			

		</div>
		<!-- :style="ind==index? '{color:'+'#ff0000'+'}':'{color:'+'#747474'+'}'" -->
		<div class="i-row i-content-center" >
			<div class="dots" v-for="(item, ind) in list"  :style="{backgroundColor: (index == ind?dotColor: dotColor2)}"></div>
			
		</div>
		<!-- <div class="swiper-dots">
			<div v-for="(item, ind) in list" :key="ind" class="dots-item">1</div>
		</div> -->
	</div>
</template>
<script>
	export default {
		...
		data() {
			return {
				...
				itemWidth: "90%", // 单个轮播图容器的宽度，默认屏幕宽度,
				swiperStyle: {}, // 控制轮播的样式
				index: 1, // 当前显示的轮播图索引，默认第一张
				touchStart: {}, // 触摸开始点位
				touchEnd: {}, // 触摸结束点位
