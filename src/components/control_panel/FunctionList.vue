        ...
        <el-row class="search_form">
          <el-form class="demo-ruleForm login_form" label-width="auto">
            ...
            <el-col :span="14" style="text-align: right;">
              ...
              <el-button icon="el-icon-plus" @click="addFunction" type="primary"
                >添加</el-button
              >
            </el-col>
          </el-form>
...
export default {
  ...
  methods: {
    ...
    async getFunctionTree() {
      const { data: res } = await this.$http.get('/manager/item/tree')
      if (res.state !== 200) {
    ...
    queryFunction() {
      if (this.filterPara.menuName !== '') {
        this.queryInfo.menuName = this.filterPara.menuName
      }

      if (this.filterPara.state !== '') {
        this.queryInfo.state = this.filterPara.state
        this.getFunctionList()
      }
    },
    ...
    async RemoveFunction() {
      const { data: res } = await this.$http.post('/manager/item/del', {
