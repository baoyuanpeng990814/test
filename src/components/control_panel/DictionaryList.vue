        ...
        <el-container>
          ...
          <el-main class="list_body" >
            <el-table  height=" calc(100vh - 290px) " ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%"
              ...
              <el-table-column prop="dictId" label="序号" width="50">
              </el-table-column>
              <el-table-column prop="dictName" label="字典类型名称" width="150">
              </el-table-column>

              <el-table-column prop="dictCode" label="字典类型编码" width="150">
              </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      async getDictionaryList() {
        ...
        const {
          data: res
        } = await this.$http.post("/manager/dict/list", this.queryInfo)
        if (res.state !== 200) {
          this.tableData = []
          //return this.$message.error('数据获取失败！')
        } else {
      ...
      async RemoveType() {
        ...
        const {
          data: res
        } = await this.$http.post("/manager/dict/del", {
