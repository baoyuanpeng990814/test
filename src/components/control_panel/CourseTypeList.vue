        ...
        <el-container>
          ...
          <el-main class="list_body" >
            <el-table  height=" calc(100vh - 350px) " ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%"
              ...
              <el-table-column prop="coursewareTypeId" label="序号" width="50">
              </el-table-column>
              <el-table-column prop="coursewareTypeName" label="课件分类名称" width="200">
              </el-table-column>
              <el-table-column prop="stutes" label="状态" width="100">
  ...
  export default {
    ...
    methods: {
      ...
      async getCourseTypeList() {
        ...
        const {
          data: res
        } = await this.$http.post("/manager/ctyp/list", this.queryInfo)
        if (res.state !== 200) {
          this.tableData = []
          //return this.$message.error('数据获取失败！')
        } else {
      ...
      async RemoveType() {
        ...
        const {
          data: res
        } = await this.$http.post("/manager/ctyp/del", {
