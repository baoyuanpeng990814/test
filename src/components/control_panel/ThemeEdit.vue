<template>
  <div id="app">
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item :to="{ path: '/themeedit' }">编辑主题</el-breadcrumb-item>
    </el-breadcrumb>
            ...
            <el-row>
              <el-col :span="24">
                <el-form-item label="组织机构" prop="createOrg">
                  <el-input v-model="themedetail.creatOrgName" @focus="organizationDialog=true"
...
<script>
  export default {
    ...
    created() {
      this.themedetail = this.$route.query.row
    ...
    methods: {
      ...
      async modifyUser() {
        const {
          data: res
        } = await this.$http.post("/manager/theme/edit", this.themedetail)
        if (res.state !== 200) {
      ...
      choose(row) {
        if (this.organizationDialog) {
          ...
          this.themedetail.creatOrgName = row.organizationName
        }
