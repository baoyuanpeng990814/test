      ...
      <div slot="header" class="clearfix">
        ...
        <el-row class="buttons">
          <el-col :span="24" :offset="0">
            <el-button icon="el-icon-plus" @click="addProgram" type="primary">添加</el-button>
          </el-col>
        </el-row>
      </div>
      <div>
        <el-container>
          ...
          <el-main class="list_body">
            <el-table  height=" calc(100vh - 350px) " ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%"
              ...
              <el-table-column prop="courseSortId" label="序号" width="60">
              </el-table-column>
              <el-table-column prop="courseSortName" label="课程名称" width="220">
              </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      async getProgramList() {
        ...
        const {
          data: res
        } = await this.$http.post("/manager/csort/list", this.queryInfo)
        if (res.state !== 200) {
          return this.$message.error('数据获取失败！')
        } else {
      ...
      async RemoveProgram() {
        const {
          data: res
        } = await this.$http.post("/manager/csort/del",{courseSortId:this.supposeDelete.courseSortId})
