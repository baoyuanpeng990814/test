            ...
            >
              <span v-for="s in submenu" :key="s.id">
                <el-menu-item v-if="s.chilLItem.length == 0" :index="s.fireUrl">
                  <i class="el-icon-user" v-show="s.menuName === '主讲人'"></i
                  ><i
                    class="el-icon-s-comment"
                    v-show="s.menuName === '消息管理'"
                  ></i
                  ><i
                    class="el-icon-s-marketing"
                    v-show="s.menuName === '轮播图管理'"
                  ></i
                  ><i
                    class="el-icon-notebook-1"
                    v-show="s.menuName === '课件管理'"
                  ></i
                  ><i
                    class="el-icon-notebook-2"
                    v-show="s.menuName === '课件分类管理'"
                  ></i
                  >&nbsp;&nbsp;{{ s.menuName }}
                </el-menu-item>
                <el-submenu :index="s.menuName" v-else>
                  <template slot="title">{{ s.menuName }}</template>
...
export default {
  ...
  methods: {
    listSubItems() {
      this.dataList = this.$store.getters.role
      var item = this.dataList.find(x => x.fireUrl === '/questionlist')
