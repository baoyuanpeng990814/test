<template>
  <div id="app">
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      <el-breadcrumb-item :to="{ path: '/studyanalyzation' }"
        >学习监控</el-breadcrumb-item
      >
    </el-breadcrumb>
        ...
        >
          ...
          <el-table-column prop="courseType" label="课程类型" width="120">
            <template slot-scope="scope">{{
              scope.row.courseType | transformType
            }}</template>
          </el-table-column>
          ...
          <el-table-column prop="studyRate" label="参学率" width="80">
            <template slot-scope="scope">{{
              scope.row.studyRate | transformDecimal
            }}</template>
          </el-table-column>
          <el-table-column prop="completeRate" label="结业率" width="80">
            <template slot-scope="scope">{{
              scope.row.completeRate | transformDecimal
            }}</template>
          </el-table-column>
...
export default {
  ...
  created() {
    let currentStudyPage = sessionStorage.getItem('currentStudyPage')
    if (currentStudyPage != null) {
      this.queryInfo.page = Number(currentStudyPage)
    }
    this.getStudyNote()
    this.getDataList()
  },
  mounted() {
    console.log(this.queryInfo,'222')
  },
  methods: {
    async getStudyNote() {
      let params = {
    ...
    handleCurrentChange(newPage) {
      sessionStorage.setItem('currentStudyPage', newPage)
