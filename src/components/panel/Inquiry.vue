<template>
  ...
  >
    <el-card class="box-card">
      ...
      <div>
        <div class="inquiry_item" v-for="(c, i) in inquiries" :key="c.id">
          <h3>问题{{ i + 1 }}</h3>
          <el-form label-width="auto">
            ...
            >
              ...
              分数
              <el-input-number
                v-model="c.examineQuestionFraction[l - 1]"
              ></el-input-number>
            </el-form-item>
            <el-button
              size="small"
            ...
            <el-button
              ...
              type="primary"
              icon="el-icon-plus"
              @click="changeOptionList(++c.length)"
              circle
            >
            </el-button>
            <el-button
...
export default {
  ...
  methods: {
    ...
    submitInquiry(val) {
      ...
      params.examineQuestionFraction = params.examineQuestionFraction.substring(
        0,
        params.examineQuestionFraction.length - 1
      )
      params.examineQuestionOption = params.examineQuestionOption.substring(
        0,
        params.examineQuestionOption.length - 1
      )
    ...
    async updateInquiry(params) {
      //修改问题
      const { data: res } = await this.$http.post(
        '/manager/examine/editQuestion',
        params
      )
      if (res.state !== 200) {
    ...
    async insertInquiry(params, val) {
      // 添加问题
      const { data: res } = await this.$http.post(
        '/manager/examine/addQuestion',
        params
      )
      if (res.state !== 200) {
    ...
    async deleteInquiry(params) {
      //删除问题
      const { data: res } = await this.$http.post(
        '/manager/examine/delQuestion',
        {
