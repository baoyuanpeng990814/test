			...
			<div slot="header" class="clearfix">
				...
				<el-row class="buttons">
					<el-col :span="24" :offset="0">
						...
						<el-button icon="el-icon-plus" @click="addnews" type="primary">添加</el-button>
					</el-col>
				</el-row>
			</div>
			<div>
            ...
            <el-tree :expand-on-click-node="false" :default-expand-all="true" :data="treedata" :props="defaultProps" @node-click="handleNodeClick">
					...
					<el-main class="list_body">
						<el-table height=" calc(100vh - 350px) " ref="multipleTable" :data="tableData"
							...
							<el-table-column prop="newId" label="序号" width="120">
							</el-table-column>
							...
							<el-table-column prop="newTitle" label="新闻标题" show-overflow-tooltip>
							</el-table-column>
	...
	export default {
		...
		methods: {
			...
			async getNewsList() {
				const {
					data: res
				} = await this.$http.post("/manager/new/list", this.queryInfo)
				if (res.state !== 200) {
					//return this.$message.error('数据获取失败！')
					this.tableData = []
				} else {
			...
			async RemoveNews() {
				const {
					data: res
				} = await this.$http.post("/manager/new/del", {
					newId: this.supposeDelete.newId
				})
				if (res.state !== 200) {
