			...
			<el-aside width="180px" class="management_side">
				...
				<div class="side_menu_bg">
					...
					<el-col :span="12" class="side_menu_bg">
						<el-menu text-color="#ffffff" default-active="2" background-color="#4A5064"
							...
							<span v-for="s in submenu">
								...
								<el-submenu :index="s.menuName" v-else>
									<template slot="title"><i
											class="el-icon-reading">&nbsp;&nbsp;{{s.menuName}}</i></template>
									<el-menu-item v-for="l in s.chilLItem" :index="l.fireUrl">{{l.menuName}}
									</el-menu-item>
								</el-submenu>
							</span>
						</el-menu>
...
<script>
	...
	export default {
		...
		methods: {
			listSubItems() {
				this.dataList = this.$store.getters.role
				var item = this.dataList.find(x => x.fireUrl == "/newslist")
